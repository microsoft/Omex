...
{
	internal class HttpEndpointHealthCheck1 : AbstractHealthCheck<HttpHealthCheckParameters1>
	{
		...
		public HttpEndpointHealthCheck1(
			HttpHealthCheckParameters1 parameters,
			IHttpClientFactory httpClientFactory,
			IAccessor<ServiceContext> accessor,
			ILogger<HttpEndpointHealthCheck1> logger,
		...
		protected override async Task<HealthCheckResult> CheckHealthInternalAsync(HealthCheckContext context, CancellationToken token)
		{
			...
			UriBuilder builder = new UriBuilder(Parameters.Scheme, Host, port, Parameters.RelativeUri.ToString(), Parameters.QueryString);

			HttpClient httpClient = m_httpClientFactory.CreateClient(HttpClientLogicalName);

			HttpRequestMessage request = Parameters.HttpRequest(builder);
