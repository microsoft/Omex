...
namespace Microsoft.Omex.System.Data
{
	...
	public class ResourceMonitor : IResourceMonitor, IDisposable
	{
		...
		{
			if (!Code.ValidateArgument(resourcesToMonitor, nameof(resourcesToMonitor), TaggingUtilities.ReserveTag(0)) ||
				!Code.ValidateArgument(callbackAction, nameof(callbackAction), TaggingUtilities.ReserveTag(0)))
			{
				return false;
			}
		...
		private void CheckResourcesForUpdates(IEnumerable<IResource> resources, ResourceMonitoringData singleMonitoringData)
		{
			...
			foreach (IResource resource in resources)
			{
				...
				if (newDetails == null)
				{
					ULSLogging.LogTraceTag(0, Categories.ConfigurationDataSet, Levels.Warning, "Failed to load resource details for '{0}'", resource.Name);
					continue;
				}
		...
		private IResourceDetails GetFileDetails(IResource resource)
		{
			...
			{
				ULSLogging.LogTraceTag(0, Categories.ConfigurationDataSet, Levels.Warning,
