...
namespace Microsoft.Omex.System.Configuration.DataSets
{
	...
	public abstract class ConfigurationDataSet<T> : IConfigurationDataSet where T : ConfigurationDataSetLogging, new()
	{
		...
		{
			if (!Code.Validate((resources?.Count ?? 0) > 0, TaggingUtilities.ReserveTag(0x2384e509 /* tag_97ouj */),
				"{0} should containt at least one resource, unable to load '{1}'", nameof(resources), GetType().Name))
			{
				...
				ULSLogging.LogCodeErrorTag(0x2384e50f /* tag_97oup */, Categories.ConfigurationDataSet, false,
		...
		protected bool LoadResource(IDictionary<string, IResourceDetails> resources, string resourceName, out byte[] resourceContent)
		{
			...
			if (!Code.ValidateArgument(resources, nameof(resources), TaggingUtilities.ReserveTag(0x2384e50a /* tag_97ouk */)) ||
				!Code.ValidateNotNullOrWhiteSpaceArgument(resourceName, nameof(resourceName), TaggingUtilities.ReserveTag(0x2384e50b /* tag_97oul */)))
			{
				return false;
			}
			...
			{
				ULSLogging.LogCodeErrorTag(0x2384e50c /* tag_97oum */, Categories.ConfigurationDataSet, false, true,
